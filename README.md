...
- **セッション分析**: スコアリングシステムによるパフォーマンスの詳細な分析を提供します。音程（ピッチ）の正確さに加え、発音タイミングの偏差（ミリ秒）も評価し、総合スコアを算出します。システム全体のレイテンシ（遅延）を自動的に補正する仕組みも備えており、ユーザーの意図したタイミングをより正確に評価します。音の立ち上がり等の過渡期ノイズを除外する **25% トリム平均** を採用。
...
- **チューナー専用画面**: モバイルデバイス等での利用を想定した、ピッチモニター機能のみを全画面表示する専用ページ（`/tuner`）を提供。不要なUI要素を排除し、視認性を最大化しています。
...
### 精度評価テストの安定性について
特に Windows 環境において、Web Audio API のタイミング精度やサンプリングレートの不一致（44.1kHz / 48kHz）によるテストの失敗を防ぐため、以下の対策が施されています：
- **テスト音源の動的生成**: ブラウザのサンプリングレートを検出し、その場で最適な WAV ファイルを再生成します。
- **リソース制限**: CPU負荷によるタイミングの揺れを最小限に抑えるため、ワーカ数を 1 に固定して実行されます。
- **ゾンビプロセス対策**: ポート競合を防ぐため、テストごとにクリーンなサーバープロセスを起動します。

これらにより、CI環境およびローカルの Windows 環境の双方で、高い再現性を持って精度評価が行われます。
