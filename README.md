...
- **AI曲データ生成 (WebGPU)**: ローカルで動作するLLM (Bonsai 1.7B) をブラウザ上で実行し、プロンプト（例：「Cメジャーのウォーキングベースを作って」）を入力するだけで、自動的にJSON形式の楽曲データを生成して練習課題として読み込むことができます。HuggingFace Transformers.js を利用しています。
