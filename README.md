...
- **オーディオ設定の保存**: 好みの入力/出力デバイスの選択、およびマスター音量（メトロノーム、デモ音源、録音再生時の音量）の設定が可能です。選択した設定はブラウザに保存（`localStorage`）され、次回アクセス時に自動復元されます。
...
- **セッション分析**: スコアリングシステムによるパフォーマンスの詳細な分析を提供します。音程（ピッチ）の正確さに加え、発音タイミングの偏差（ミリ秒）も評価し、総合スコアを算出します。システム全体のレイテンシ（遅延）を自動的に補正する仕組みも備えており、ユーザーの意図したタイミングをより正確に評価します。
- **デモ再生機能**: 選択中の楽曲の構成音（小数ビートの音符含む）を Web Audio API によるサイン波でプレビュー再生し、正しいリズムと音程を耳で事前に確認できます。
- **フリー採点モード**: 固定の譜面なしで自由に演奏し、イントネーションの正確さと安定度をリアルタイムで測定・記録します。スライド移動によるノイズの自動除外（スライド検知）にも対応しています。フリーモードでの「再生」時は、メトロノームの連続クリックと録音された音声が同時に再生されます。
- **録音と再生**: `MediaRecorder` を使用して演奏を WebM 形式で録音可能。録音データは「再生」モード時にメトロノームと一緒に同期再生でき、ブラウザから直接ダウンロードすることも可能です。
- **曲のリピート機能**: 曲全体を指定した回数だけ繰り返して練習できます。リピート中は現在の周回数（n/m）が表示されます。
- **多段スコア表示**: 五線譜・タブ譜を4小節/行で多段表示。統合レイアウトエンジンにより五線譜とタブ譜の描画位置が完璧に同期。臨時記号（♯/♭）の自動描画、キー（調）の明示、オクターブ番号を省いたシンプルな音名表記に対応。また、演奏された音符は評価結果（perfect, good, ok, miss）に応じてリアルタイムに色分けされ、タイミングのズレ（早い・遅い）に応じて左右に表示がブレる視覚的フィードバックも備えています。
