...
- **E2Eテスト** (`tests/e2e/`): ブラウザを介したUI/UXおよび精度のテストです。**Playwright** を使用して記述します。`evaluation.spec.ts` では生成されたテスト用音源を用いて、ピッチ **80%以上**、タイミング **70%以上**（1音目を除いた平均で80%以上に相当）の精度が出ることを自動検証します。また、無音入力時にスコアが 0% になることも検証されます。
