...
- **E2Eテスト**: `evaluation.spec.ts` での精度検証時、下記のように設定を行います（プラットフォームによって最適なレイテンシ補正値が異なります）。
  ```typescript
  const songName = await page.evaluate(() => {
    ...
    // Windows: 195ms, macOS: 260ms (Chromium fake audio captureの遅延の差異)
    audioState.latencyCompensationMs = (navigator.platform.indexOf('Win') > -1) ? 195 : 260; 
...
- **E2Eテスト** (`tests/e2e/`): ブラウザを介したUI/UXおよび精度のテストです。**Playwright** を使用して記述します。`evaluation.spec.ts` では生成されたテスト用音源を用いて、ピッチ・タイミング共に **80%以上** の精度が出ることを自動検証します。また、無音入力時にスコアが 0% になることも検証されます。
...
メトロノームに合わせた演奏時に発生するシステム全体の遅延（出力レイテンシおよび入力レイテンシ）を補正するため、`audioState` に `latencyCompensationMs` を保持しています。
- **デフォルト値**: 250ms。
- **E2Eテスト環境**: Chromium の `fake-audio-capture` を使用する場合、OSによって遅延が異なります。Windows では **195ms**、macOS では **260ms** が現在の最適値として `evaluation.spec.ts` に設定されています。
