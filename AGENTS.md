...
- **`setSinkId` の使用禁止**: スピーカーなどの出力先をプログラムから切り替える `setSinkId` は、一部の仮想デバイス構成（Voicemeeter等）において非同期的にブラウザのオーディオレンダラプロセス全体をクラッシュさせる致命的な不具合があります（`try-catch` でも回避不可）。そのため、本プロジェクトではプログラムからの出力ルーティング（`setSinkId`）を原則禁止・無効化し、OSの既定出力デバイスへそのまま流す仕様としています。
