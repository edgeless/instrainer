...
- **自動デプロイ (CI/CD)**:
  - GitHub Actions を用いて、`main` ブランチにプッシュ（マージ）された際に、型チェック、ユニットテスト、E2E テストが実行され、すべて成功した場合に Cloud Run へ自動デプロイされます。
  - 自動デプロイの設定は `.github/workflows/test.yml` に記述されています。
