...
`Transport.svelte` 内のリアルタイム判定（`gradeNote`）や録音終了後の再計算（`runPostAnalysis`）では、検出された発音時刻からこの補正値を減算することで、ユーザーが意図した正確な打鍵タイミングを算出しています。

**E2E環境における補正**:
E2Eテスト環境では、仮想オーディオパイプラインの特性に合わせ、`latencyCompensationMs` をテストコード側で適切に設定（キャリブレーション）することで、シミュレートされた音源とアプリのタイミングを同期させています。
