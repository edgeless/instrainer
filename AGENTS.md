...
### E2Eテスト実行時の注意点と知見
- **Headless環境の仮想オーディオ**: Playwrightの仮想マイクループバック（`--use-fake-ui-for-media-stream`, `--use-file-for-fake-audio-capture`）では、物理デバイスとは異なるタイミング特性やレイテンシを持ちます（約100ms程度）。テストシナリオ内では `window.__states.audioState` を介して `latencyCompensationMs` を動的に上書きしてテストの安定化を図っています。
- **Headedモードと並列実行**: `--headed` フラグで画面を表示しながらテストを行う場合、Playwrightのデフォルトの複数ワーカー（並列実行）では「Target page, context or browser has been closed」というエラーでクラッシュすることがあります。Headedモード時は `--workers=1` を指定して直列実行することを推奨します。
- **ポートの競合**: `playwright.config.ts` の `reuseExistingServer` の設定によっては、Viteのプロセス（ポート5173）が残留しテストが失敗するケースがあります。その場合は `taskkill` などで手動でプロセスを終了させるか、CI環境同様にクリーンな状態で起動してください。
...
メトロノームに合わせた演奏時に発生するシステム全体の遅延（出力レイテンシおよび入力レイテンシ）を補正するため、`audioState` に `latencyCompensationMs`（現状は固定値の 250ms）を保持しています。
`Transport.svelte` 内のリアルタイム判定（`gradeNote`）や録音終了後の再計算（`runPostAnalysis`）では、検出された発音時刻からこの補正値を減算することで、ユーザーが意図した正確な打鍵タイミングを算出しています。今後はユーザーごとにキャリブレーションできる機能への拡張が予定されています。
※E2Eテスト環境（Headless Chromium）では、この遅延が物理デバイスとは異なるため、テストコード側で動的に補正値（100ms等）を注入してテストを安定させています。

## 16. ピッチ検出アルゴリズムの特性と対策 (YIN)

本アプリでは、軽量かつリアルタイム性に優れた**YINアルゴリズム**（`src/lib/utils/pitch.ts`）を用いてピッチ検出を行っていますが、ベースギター特有の低音域における安定性を確保するため、以下の対策を実装しています。

- **Octave-Agnostic（オクターブ非依存）な判定**: YINアルゴリズムは信号のノイズや倍音成分により、本来のピッチの1オクターブ上（または下）の周波数を「誤検出（Octave Doubling/Halving）」しやすい性質があります。これを許容するため、`freqToCents` 関数内で検出周波数をターゲット周波数のオクターブ内に収まるよう自動調整（`while d > target * 1.5 d /= 2` など）してから評価を行っています。ベースの練習用途においては、倍音を拾ったことによる判定ミスを防ぐ実用的なアプローチです。
- **ディップの閾値調整**: 基本周波数の誤検出を減らすため、差分関数のディップを判定する `threshold` は標準的な `0.10 ~ 0.15` よりもやや厳しい `0.08` を採用しています。
